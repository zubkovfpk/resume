...
def build_story() -> list:
    ...
    story.append(Paragraph(
        "20+ лет в ИТ и цифровой трансформации, из них последние 7+ лет — фокус на AI/ML-продукты "
        "и стратегию AI-направления. Co-Founder и CTO компании ВИЗАРД: выстроил AI-направление с нуля — "
        "от исследования рынка и валидации гипотез до запуска ML-продуктов в production и масштабирования "
        "на крупных корпоративных клиентов. Hands-on архитектор и разработчик: LLM, Agentic RAG, GraphRAG, "
        "multi-agent workflows, CV/ML на данных БПЛА. Умею работать в неопределённости, быстро тестировать "
        "гипотезы и переводить AI-возможности в измеримый бизнес-результат. "
    ...
    story.append(metrics_row([
        ("7+",       "лет в AI/ML"),
        ("2",        "AI-продукта в production"),
        ("1 млрд ₽+","экономия клиентов VIZARD"),
        ("mAP ≥ 80%","ML-детекция (БПЛА)"),
    ]))
    ...
    skill_cards = [
        ...
        ("Продуктовое мышление",
         "Работа с кросс-функциональными командами (продукт, маркетинг, e-commerce, аналитика). "
         "Customer journey, UX, влияние AI на пользовательский опыт."),
    ...
    vizard_bullets = [
        (highlight("AI-направление с нуля:") +
         ...
         "запускал R&D-проекты — от идеи до пилота и масштабирования на крупных корпоративных клиентах "
         "(ПАО «Газпром», ПАО «Совкомфлот», ПАО «НОВАТЭК» и др.)."),
        (highlight("AI-продукты в production:") +
         " ML-продукты (CV/YOLO/DETR, детекция и классификация объектов), LLM-интеграции, "
         "Agentic RAG / GraphRAG, multi-agent workflows. "
         "Оценка эффективности через бизнес-метрики клиентов."),
        (highlight("Hands-on кейс — метеобюллетень:") +
         " лично спроектировал и развиваю AI-driven модуль автоматизированной подготовки "
         "гидрометеорологических бюллетеней (Python, GFS/CMEMS, multi-agent AI, CI/CD). "
         "Репозиторий GitHub — доступ по запросу."),
        (highlight("Hands-on кейс — ML / БПЛА (Россельхознадзор):") +
         " R&D и MVP по компьютерному зрению для детекции сорных растений на аэрофотоснимках БПЛА "
         "(YOLO/EfficientDet/DETR/RT-DETR; mAP@0.5 ≥ 80%, IoU ≥ 0.75, F1 ≥ 0.8). "
         "MVP сдан заказчику; идут процедуры промышленного запуска."),
        (highlight("Кросс-функциональная работа:") +
         " взаимодействие с командами продукта, маркетинга, продаж и внешними стейкхолдерами; "
    ...
    story.append(KeepTogether(
        exp_header_with_tags(
            "Chief Architect / Scenario Business Architect",
            "2012 — 2016",
            "Единое геоинформационное пространство Москвы (ЕГИП) — Департамент информационных технологий г. Москвы",
            ["Цифр. трансформация", "Data-платформа", "B2C / B2G"]
        ) + [Spacer(1, 2)]
    ))
    story.append(bullet(
        highlight("Городская data-платформа:") +
        " 60+ органов власти, 37 интегрированных систем, аудитория — миллионы жителей Москвы; "
        "ML первого поколения (детекция объектов на LiDAR + стереофото)."
    ))
    story.append(bullet(
        "Снижение TCO в 6,3×; экономия бюджета города >0,5 млрд руб./год; "
