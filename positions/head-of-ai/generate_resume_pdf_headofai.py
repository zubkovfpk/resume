...
def build_story() -> list:
    ...
    roskad_bullets = [
        ...
        (highlight("Дорожная карта GeoAI 2026–2030:") +
         " разработал стратегический документ внедрения ИИ на федеральном уровне "
         "(6 блоков, 25 мероприятий, горизонт 5 лет): "
         "change detection по ДЗЗ/БПЛА (целевой F1 ≥ 0.85), семантическая сегментация объектов местности (mIoU ≥ 0.80), "
         "risk-engine с ГИС/ИПД, система раннего предупреждения (≤24 ч). "
         "MLOps-контур: мониторинг деградации моделей (model drift / data drift), "
         "валидация по ГОСТ Р 57773-2017, dataset registry на базе НСПД."),
    ]
